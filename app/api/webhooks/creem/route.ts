...
// Sits under app/api/webhooks/ alongside the Paddle one so it inherits the
// middleware exclusion in proxy.ts — a webhook carries no session cookie, so the
// refresh is wasted work, and its guard is in the route, not in middleware.
export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/**
 * Creem webhook. Same security model, same order of operations, as the Paddle
 * route next door:
 ...
 * silently drop real deliveries the day Creem changed egress. This matches the
 * Paddle route, which is likewise signature-only.
 *
 ...
 * Creem's per-delivery event id — the direct analog of Paddle's `event_id`, and
 * the key the creem_events primary key dedupes on. Driving grants from
