  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="overflow-hidden rounded-md border border-line">
        {tools.map((tool, i) => (
          <div key={tool.id} className="flex items-center gap-3 border-b border-line px-4 py-3 last:border-0">
            <div className="flex flex-col">
              <button onClick={() => move(i, -1)} disabled={i === 0 || pending} className="text-text-faint hover:text-text disabled:opacity-30" aria-label="Move up">
                <ChevronUp aria-hidden className="size-4" strokeWidth={1.5} />
              </button>
              <button onClick={() => move(i, 1)} disabled={i === tools.length - 1 || pending} className="text-text-faint hover:text-text disabled:opacity-30" aria-label="Move down">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                <span className="truncate text-body text-text">{tool.name}</span>
        ...
        {tools.length === 0 && (
          <p className="px-4 py-8 text-center text-small text-text-faint">No tools yet. Create the first.</p>
        )}
      </div>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="fixed inset-0 bg-backdrop" onClick={() => setConfirmDelete(null)} aria-hidden />
          <div role="dialog" aria-modal="true" className="relative w-full max-w-[420px] rounded-lg border border-line bg-elevated p-5 shadow-modal">
            ...
            {confirmDelete.run_count > 0 ? (
              ...
              <p className="mt-2 text-small text-text-muted">This draft has no runs. It&apos;ll be gone for good.</p>
            )}
