 ...
 * straight from the tools table, so it grows every time a tool ships.
 *
 * Rail alignment: the `pl-9` lives on each <li>, NOT the <ol>. That way the node
 * (`left-[9px]` inside the li) and the rail (`left-[9px]` inside the ol) share
 * the same reference and land on the same line; the li's padding only pushes the
 * card to the right.
 */
export function ShippingLog({ tools }: { tools: ToolCardData[] }) {
  ...
  return (
    <ol className="relative flex flex-col gap-4">
      ...
      <span aria-hidden className="absolute bottom-8 left-[9px] top-8 w-px bg-line" />
      ...
      {tools.map((tool, i) => {
        ...
        return (
          <li key={tool.slug} className="relative pl-9">
            ...
            <div
              className={cn(
                "flex flex-col gap-4 rounded-[16px] border p-4 transition-colors duration-micro ease-default sm:flex-row sm:items-start sm:gap-6 sm:p-5",
                newest
